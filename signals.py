...
def _classify_ride_structure_phase_info(
    *,
    ...
    try:
        ...
        c_struct = pd.to_numeric(struct_ctx['close'], errors='coerce').dropna()
        ...
        struct_range_atr = float((float(h_struct.max()) - float(l_struct.min())) / atr_val) if min(len(h_struct), len(l_struct)) >= 5 else short_range_atr
        ...
        if direction == 'LONG':
            ...
            trend_step = float(np.clip(np.mean(np.diff(l_short.tail(min(6, len(l_short))).values) >= -0.03 * atr_val), 0.0, 1.0)) if len(l_short) >= 4 else close_stack
        else:
            lower_wicks = (np.minimum(c.values, o.values) - l.values) / ranges
            ...
            trend_step = float(np.clip(np.mean(np.diff(h_short.tail(min(6, len(h_short))).values) <= 0.03 * atr_val), 0.0, 1.0)) if len(h_short) >= 4 else close_stack

        compression_build = bool(
            defended
            and (compressed or squeeze_ratio <= 0.74)
            and 0.08 <= dist_break <= 0.72
            and close_stack >= 0.52
            and reject <= 0.32
            and struct_extension <= 1.45
            and trend_step >= 0.50
        )
        true_extension = bool(
            dist_break >= 0.75
            and struct_extension >= 1.35
            and (
                range_atr >= 1.20
                or squeeze_ratio >= 0.92
                or reject >= 0.22
                or body_q <= 0.46
                or giveback >= 0.16
            )
        ...
        if failed:
            ...
            detail_phase = 'COMPRESSION_BUILDUP'
            conf = float(np.clip(0.52 + 0.16 * close_stack + 0.10 * trend_step + 0.10 * max(0.0, 0.90 - squeeze_ratio) + 0.08 * max(0.0, 0.34 - reject), 0.0, 0.96))
            interp = 'Pressure is building above support and may resolve through continuation breakout'
            ...
            conf = float(np.clip(0.58 + 0.18 * close_stack + 0.10 * body_q + 0.08 * trend_step + 0.08 * (0.30 - reject), 0.0, 0.99))
            ...
            conf = float(np.clip(0.52 + 0.14 * min(1.2, dist_break) + 0.10 * min(1.0, struct_extension / 1.8) + 0.10 * max(0.0, reject - 0.18) + 0.08 * max(0.0, 0.52 - body_q), 0.0, 0.96))
            interp = 'Trend remains healthy but the move is stretched enough to favor a reset before continuation'
        else:
            detail_phase = 'UNSTRUCTURED'
            conf = float(np.clip(0.34 + 0.10 * defended + 0.08 * close_stack + 0.06 * trend_step, 0.0, 0.82))
...
def _ride_macd_phase_utility(
    *,
    ...
    try:
        ...
        if phase == 'BREAK_AND_HOLD':
        ...
        elif phase == 'COMPRESSION_BUILDUP':
            if state == 'IGNITING':
            ...
            if state == 'ACCELERATING':
                ...
                score_adj -= 12.0; hard_block = True
        elif phase == 'EXTEND_THEN_PULLBACK':
            if state == 'ACCELERATING':
                score_adj += 1.0
            elif state == 'TAPERING':
        ...
        if state == 'COUNTER_TREND':
            score_adj -= 8.0
            if phase in ('BREAK_AND_HOLD', 'EARLY_ACCEPTANCE', 'PERSISTENT_CONTINUATION', 'COMPRESSION_BUILDUP') and mode == 'BREAKOUT':
                hard_block = True
        out.update({
...
def compute_scalp_signal(
    ...
    **_ignored: object,
    ...
    # Compression breakout detection: catch stored-pressure releases that are not fresh
    # reclaim events but do represent a real squeeze → expansion move.
    scalp_compression_long = _assess_compression_breakout("LONG", df, atr_last, lookback=6, break_trigger=float(orb_high) if orb_high is not None else None)
    scalp_compression_short = _assess_compression_breakout("SHORT", df, atr_last, lookback=6, break_trigger=float(orb_low) if orb_low is not None else None)
    compression_breakout_long = bool(scalp_compression_long.get("ready") or False)
    compression_breakout_short = bool(scalp_compression_short.get("ready") or False)
    ...
    primary_trigger = bool(vwap_event or rsi_event or macd_event or pro_trigger or early_pre_long or early_pre_short or ignition_override_long or ignition_override_short or tape_pre_long_assist or tape_pre_short_assist or compression_breakout_long or compression_breakout_short)
    ...
    pre_long_ok = bool(
        ...
        and isinstance(last_price, (float, int))
        and float(last_price) < float(ref_vwap)
        and (trigger_near or early_pre_long or ignition_override_long or tape_pre_long_assist or compression_breakout_long)
        and (rsi_event or rsi_pre_long or macd_event or macd_pre_long or pro_trigger_long or ignition_override_long or compression_breakout_long or (macd_build_long_pts >= 4))
        and (struct_pre_long or liquidity_event_long or orb_event_long or reclaim_attempt_long or ignition_override_long or compression_breakout_long)
        and (long_confirmation_score >= max(2, effective_confirm_threshold - 1) or early_pre_long or ignition_override_long or tape_pre_long_assist or compression_breakout_long)
    )
    pre_short_ok = bool(
        isinstance(ref_vwap, (float, int))
        and isinstance(last_price, (float, int))
        and float(last_price) > float(ref_vwap)
        and (trigger_near or early_pre_short or ignition_override_short or tape_pre_short_assist or compression_breakout_short)
        and (rsi_event or rsi_pre_short or macd_event or macd_pre_short or pro_trigger_short or ignition_override_short or compression_breakout_short or (macd_build_short_pts >= 4))
        and (struct_pre_short or liquidity_event_short or orb_event_short or reclaim_attempt_short or ignition_override_short or compression_breakout_short)
        and (short_confirmation_score >= max(2, effective_confirm_threshold - 1) or early_pre_short or ignition_override_short or tape_pre_short_assist or compression_breakout_short)
    )
...
def compute_ride_signal(
    ...
    **_ignored: object,
    ...
    try:
        if phase_preview_detail in ('COMPRESSION_BUILDUP', 'BREAK_AND_HOLD', 'EARLY_ACCEPTANCE'):
            phase_disp_floor = 1.05
        elif phase_preview_detail == 'PERSISTENT_CONTINUATION':
            phase_disp_floor = 1.10
        elif phase_preview_detail in ('MATURE_ACCEPTANCE', 'EXTEND_THEN_PULLBACK', 'FAILED_EXTENSION'):
            phase_disp_floor = 1.25
    except Exception:
        phase_disp_floor = 1.20
    disp_q = float(np.clip((disp_ratio - max(0.88, phase_disp_floor - 0.15)) / 1.45, 0.0, 1.0))
    disp_q = float(np.clip(disp_q + 0.04 * np.clip(phase_preview_conf, 0.0, 1.0) * float(phase_preview_detail in ('COMPRESSION_BUILDUP', 'BREAK_AND_HOLD', 'EARLY_ACCEPTANCE')), 0.0, 1.0))
    ...
    try:
        if phase_preview_detail in ('COMPRESSION_BUILDUP', 'BREAK_AND_HOLD', 'EARLY_ACCEPTANCE'):
            ...
            close_stack_w += 0.02
            retest_w -= 0.03
        elif phase_preview_detail in ('MATURE_ACCEPTANCE', 'EXTEND_THEN_PULLBACK'):
            persistence_w += 0.04
            retest_w += 0.02
            disp_w -= 0.03
    ...
    try:
        if phase_preview_detail in ('COMPRESSION_BUILDUP', 'BREAK_AND_HOLD', 'EARLY_ACCEPTANCE') and float(phase_preview_conf) >= 0.60:
    ...
    def _build_ride_entry_geometry(conviction_score: float) -> dict[str, object]:
        ...
        elif structure_phase_detail_local == 'COMPRESSION_BUILDUP':
        ...
        if direction == "LONG":
            ...
            stop_mult_local = 0.55 if stage == "PRE" else 0.80
            stop_local = float(pullback_entry_local + stop_mult_local * atr_last)
        prefers_breakout_phase_local = bool(structure_phase_detail_local in ("BREAK_AND_HOLD", "EARLY_ACCEPTANCE", "PERSISTENT_CONTINUATION", "COMPRESSION_BUILDUP"))
        ...
        try:
            ...
            elite_runaway_local = bool(
                ...
                and (structure_phase_detail_local in ("BREAK_AND_HOLD", "EARLY_ACCEPTANCE", "PERSISTENT_CONTINUATION", "COMPRESSION_BUILDUP"))
                and (not bool(multibar_extension_profile_local.get("path_stretched") or False))
                and (not bool(multibar_extension_profile_local.get("stalling") or False))
        ...
        if actionable_local:
            ...
            if tape_mode_enabled:
                ...
                try:
                    ...
                    standard_continuation_ok_local = bool(
                        ...
                        and (structure_phase_detail_local in ("BREAK_AND_HOLD", "EARLY_ACCEPTANCE", "PERSISTENT_CONTINUATION", "COMPRESSION_BUILDUP", "EXTEND_THEN_PULLBACK", "MATURE_ACCEPTANCE"))
                ...
                try:
                    ...
                    ignition_continuation_score_local = float(np.clip(
                        0.14 * float(structure_phase_detail_local == 'EARLY_ACCEPTANCE') + 0.10 * float(structure_phase_detail_local == 'COMPRESSION_BUILDUP')
                        + 0.18 * float(np.clip(structure_phase_confidence_local, 0.0, 1.0))
                    ...
                    ignition_continuation_ready_local = bool(
                        ...
                        and (not near_pullback_local)
                        and (not stale_breakout_local)
                        and str(structure_phase_detail_local) in ('EARLY_ACCEPTANCE', 'COMPRESSION_BUILDUP')
            ...
            breakout_fresh_phase_local = bool(str(structure_phase_detail_local) in ("BREAK_AND_HOLD", "EARLY_ACCEPTANCE", "PERSISTENT_CONTINUATION", "COMPRESSION_BUILDUP"))
            ...
            phase_momentum_ready_local = bool(
                ...
                and breakout_proximity_ratio_local <= (1.28 if str(structure_phase_detail_local) in ("PERSISTENT_CONTINUATION", "COMPRESSION_BUILDUP") else 1.18)
                and (
    ...
    score = _cap_score(pts + ride_zone_adj + ride_cont_strength_adj + adx_modifier + provisional_cont_phase3_adj + float(provisional_macd_phase.get("score_adj") or 0.0))
    ...
    score = _cap_score(pts + ride_zone_adj + ride_cont_strength_adj + adx_modifier + ride_cont_phase3_adj + float(ride_macd_phase.get('score_adj') or 0.0))
    ...
    if bool(ride_macd_phase.get('soft_caution') or False) and str(entry_mode or '').upper() == 'BREAKOUT' and str(structure_phase or '').upper() in ('BREAK_AND_HOLD', 'ACCEPT_AND_GO', 'EXTEND_THEN_PULLBACK'):
        if actionability and float(final_geometry.get('dist_br') or 0.0) > 0.0:
